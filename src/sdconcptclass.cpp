   ...
   setup *result) THROW_DECL
{
   ...
   if (parseptr->concept->arg1 == part_key_paranoid) {
      ...
      struct paranoid_thing {
         ...
         call_with_name *save_subject_call;
         ...
         void do_subject_call() {
            ...
            if (!(result->cmd.cmd_fraction.flags & CMD_FRAC_BREAKING_UP))
               result->cmd.cmd_fraction.fraction = pzzz->get_fracs_for_this_part();
      ...
      p.result = result;
      p.pzzz = &zzz;
      p.save_subject_call = ss->cmd.parseptr->call;
      p.saved_selector = current_options.who;
      ...
      if (ss->cmd.cmd_fraction.flags == 0) {
         ...
         p.saved_last_flagmisc = result->result_flags.misc &
            (RESULTFLAG__DID_LAST_PART|RESULTFLAG__PARTS_ARE_KNOWN);
         ...
         do_call_in_series_simple(result);
         result->result_flags.misc |= p.saved_last_flagmisc;
      }
      else {
      ...
      result->cmd.callspec = p.save_subject_call;
      ss->cmd.parseptr->call = p.save_subject_call;
      ss->cmd.parseptr->call_to_print = p.save_subject_call;
      current_options.who = p.saved_selector;
   }
